...
for (a in c(1:length(probability_thresholds))){
  ...
  # TP_freq = frequency_table$Freq[frequency_table$all_values == "TP"]
  # FP_freq = frequency_table$Freq[frequency_table$all_values == "FP"]
  # TN_freq = frequency_table$Freq[frequency_table$all_values == "TN"]
  # FN_freq = frequency_table$Freq[frequency_table$all_values == "FN"]
  
  # if (length(TP_freq) == 0 & length(FP_freq) == 0){
  #   UA = -999
  # } else {
  #   UA = TP_freq / (TP_freq + FP_freq)
  # }
  # 
  # if (length(TP_freq) == 0 & length(FN_freq) == 0){
  #   PA = -999
  # } else {
  #   PA = TP_freq / (TP_freq + FN_freq)
  # }
  # UA_list = c(UA_list, UA)
  # PA_list = c(PA_list, PA)
