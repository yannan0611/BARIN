...
# Output folder path
bfast_output_folder_path_1 <- "/Users/yannanwang/Library/CloudStorage/OneDrive-UniversityofGuelph/Desktop/M1.4/LargeDam_Data/Output/BFAST_1"
bfast_output_folder_path_2 <- "/Users/yannanwang/Library/CloudStorage/OneDrive-UniversityofGuelph/Desktop/M1.4/LargeDam_Data/Output/BFAST_2"
bfast_output_folder_path_3 <- "/Users/yannanwang/Library/CloudStorage/OneDrive-UniversityofGuelph/Desktop/M1.4/LargeDam_Data/Output/BFAST_3"
bfast_output_folder_path_4 <- "/Users/yannanwang/Library/CloudStorage/OneDrive-UniversityofGuelph/Desktop/M1.4/LargeDam_Data/Output/BFAST_4"

beast_output_folder_path_1 <- "/Users/yannanwang/Library/CloudStorage/OneDrive-UniversityofGuelph/Desktop/M1.4/LargeDam_Data/Output/BEAST_1"
beast_output_folder_path_2 <- "/Users/yannanwang/Library/CloudStorage/OneDrive-UniversityofGuelph/Desktop/M1.4/LargeDam_Data/Output/BEAST_2"
beast_output_folder_path_3 <- "/Users/yannanwang/Library/CloudStorage/OneDrive-UniversityofGuelph/Desktop/M1.4/LargeDam_Data/Output/BEAST_3"
beast_output_folder_path_4 <- "/Users/yannanwang/Library/CloudStorage/OneDrive-UniversityofGuelph/Desktop/M1.4/LargeDam_Data/Output/BEAST_4"

# Output file name prefix (CHANGE THIS FOR EACH CLASS!!!!!)
bfast_prefix <- "BFAST_NewLarge.tif"
beast_prefix <- "BEAST_NewLarge.tif"

# NA Threshold for BEAST and BFAST
pr_threshold <- 0.1 # Probability threshold used for BEAST
nathresh <- 350 # 1200 is used for Algonquin Site for BFAST
nbreaks <- 5 # Number of breaks extracted
...
MNDWI_Characterization_BFAST_1 <- function (x) {
  ...
  df <- data.frame(break_magnitude = c(magnitude), breakdate = c(breakdates))
  
  ########## Positive Break Dates ############# 
  filtered_df_pos <- df[df$break_magnitude > 0, ]
  sorted_df_pos <- filtered_df_pos[order(filtered_df_pos$break_magnitude, decreasing = TRUE),]
  
  final_magnitude_list_pos <- c(sorted_df_pos$break_magnitude, rep(NA, nbreaks))
  final_date_list_pos <- c(sorted_df_pos$breakdate, rep(NA, nbreaks))
  
  final_magnitude_pos <- final_magnitude_list_pos[1:nbreaks]
  final_date_pos <- final_date_list_pos[1:nbreaks]
  
  ########## Negative Break Dates #############
  filtered_df_neg <- df[df$break_magnitude < 0, ]
  sorted_df_neg <- filtered_df_neg[order(filtered_df_neg$break_magnitude, decreasing = FALSE),]
  
  final_magnitude_list_neg <- c(sorted_df_neg$break_magnitude, rep(NA, nbreaks))
  final_date_list_neg <- c(sorted_df_neg$breakdate, rep(NA, nbreaks))
  
  final_magnitude_neg <- final_magnitude_list_neg[1:nbreaks]
  final_date_neg <- final_date_list_neg[1:nbreaks]
  
  res <- c(final_magnitude_pos, final_date_pos, final_magnitude_neg, final_date_neg)
  return(res)
}
...
MNDWI_Characterization_BEAST_1 <- function (x) {
  ...
  neg_trend_cpPr = out$trend$neg_cpPr
  neg_trend_cpDate = out$trend$neg_cp
  
  pos_trend_cpPr[is.nan(pos_trend_cpPr)] <- 0
  neg_trend_cpPr[is.nan(neg_trend_cpPr)] <- 0
  ...
  if (length(neg_trend_cpDate) > 0){
    for (i in c(1:nbreaks)){
      if (neg_trend_cpPr[i] < pr_threshold){
        neg_trend_cpPr[i] <- NA
        neg_trend_cpDate[i] <- NA
      }
    }
  } else{
    neg_trend_cpPr <- rep(0, nbreaks)
    neg_trend_cpDate <- rep(0, nbreaks)
  }
  
  res <- c(pos_trend_cpPr, pos_trend_cpDate, pos_trend_cpPr, neg_trend_cpDate)
